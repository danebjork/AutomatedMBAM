...
class HDF5_JSON:
    ...
    def __init__(self, file_name):
        self.file_name = re.sub(r'\.h5$', '', file_name)
        ...
        for group in self.file.walk_groups():
            ...
            for array in self.file.list_nodes(group, classname="Array"):
            ...
            for gp in self.file.list_nodes(group, classname="Group"):
                gp_name = gp._v_name
                print(self.groupContentsDict)
                gp_contents = {gp_name : self.groupContentsDict[gp_name]}
                self.groupContentsDict[name].update(gp_contents)

            for table in self.file.list_nodes(group, classname="Table"):
    ...
    def jsonString(self):
        ...
        """
        alpha = self.groupContentsDict
        j = json.dumps(alpha['/'], cls=NumpyAwareJSONEncoder, ensure_ascii=False)
        self.file.close()
        return j
