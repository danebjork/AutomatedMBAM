...
module MA_IFFLP_Model

import Models
import ParametricModels
using Parameters

@with_kw type MA_IFFLP{T<:Real} <: ParametricModels.AbstractParameterSpace{T} @deftype T
	A_init = 0.0
	A_total = 1.0
	B_init = 0.0
	B_total = 1.0
...
function inp{T<:Real}(ps::MA_IFFLP{T}, _t)
	return T[]
end

function ic{T<:Real}(ps::MA_IFFLP{T})
	return T[ps.A_init, ps.B_init, ps.C_init]
end

function rhs{T<:Real}(ps::MA_IFFLP{T}, _t, _x, _dx)
	A_inact = -_x[1] + ps.A_total
	B_inact = -_x[2] + ps.B_total
	C_inact = -_x[3] + ps.C_total
	_inp = inp(ps, _t)
	_dx[1] = A_inact.*ps.k_1./(A_inact + ps.K_1) - _x[1].*ps.k_2./(_x[1] + ps.K_2)
	_dx[2] = B_inact.*_x[1].*ps.k_3./(B_inact + ps.K_3) - _x[2].*ps.k_4./(_x[2] + ps.K_4)
	_dx[3] = C_inact.*_x[1].*ps.k_5./(C_inact + ps.K_5) - _x[2].*_x[3].*ps.k_6./(_x[3] + ps.K_6)
	nothing
end

function obs{T<:Real}(ps::MA_IFFLP{T}, _t, _x)
	return T[_x[3]]
end
...
data = ParametricModels.OLSData("MA_IFFLP", ydata)
parametricmodel = @ParametricModels.ODEModel(data, MA_IFFLP, ic, rhs, obs, _t, (), Tuple{Symbol, Any}[])
...
for p in parametricmodel.parameters
	if p.s in [:K_1, :K_2, :K_3, :K_4, :K_5, :K_6, :k_1, :k_2, :k_3, :k_4, :k_5, :k_6]
		p.t = exp
		p.invt = log
	elseif p.s in []
		p.t = sinh
